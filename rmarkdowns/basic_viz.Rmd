...
```{r}
ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) + geom_point()
```
...
```
```{r}
ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = goal_5_score)) + geom_point()
```
...
```{r}
ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_6_score)) + geom_point()
```

```{r}
ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_6_score, color = regions_used_for_the_sdr)) + geom_point()
```
Interpretation: 
In this scatterplot, the relationship between "Goal 3" (good health and well-being) and "Goal 6" (clean water and sanitation) scores is visualized across different regions. There is a general positive correlation, indicating that countries performing well on health-related SDG indicators also tend to perform well on access to clean water and sanitation. The OECD region tends to cluster in the higher score range, while Sub-Saharan Africa exhibits lower scores.
...
```
```{r}
ggplot(lac_sdr_data, aes(x= goal_2_score, y = country)) +
  geom_bar(stat = "identity") 
```
...
```
Interpretation:
This histogram shows the distribution of "Goal 3" (good health and well-being) scores across different regions as indicated by various colors. The x-axis represents the "Goal 3" score, while the y-axis shows the count of countries within each score range. The histogram reveals that countries in Sub-Saharan Africa tend to have lower scores, mostly below 50. OECD countries dominate the higher score ranges, particularly between 75 and 100. Several East & South Asia and Latin American countries are spread across the middle range.
