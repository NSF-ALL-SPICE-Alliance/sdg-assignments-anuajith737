...
goal_4_histogram <- ggplot(sdr_data, aes(x = goal_4_score, fill=regions_used_for_the_sdr)) +
  ...
  theme_minimal() +
  scale_fill_viridis_d(option = "magma") +
  labs(title = "Distribution of SDG 4 Scores",
       x = "SDG 4 Score",
       y = "Number of Countries",
       fill = "Region")
ggplotly(goal_4_histogram)
...
```{r}
goal_3_goal_4_scatterplot <- ggplot(sdr_data, aes(x = goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal() +
  scale_fill_viridis_d(option = "cividis") +
  labs(title = "SDG 3 and SDG 4 Relationship",
       x = "SDG 3 Score",
       y = "SDG 4 Score",
       Fill = "Region")
...
```{r}
mytext <- paste("Country: ", sdr_data_world_joined$country, "<br/>", "Goal 7 score: ", round(sdr_data_world_joined$goal_7_score, 2),
                sep = "") %>%
  lapply(htmltools::HTML)
...
```{r}
mytext <- paste("Country: ", sdr_data_world_joined$country, "<br/>", "Goal 3 score: ", round(sdr_data_world_joined$goal_3_score, 2),
                sep = "") %>%
  lapply(htmltools::HTML)
...
leaflet(sdr_data_world_joined) %>%
  addTiles() %>%
  setView( lat=10, lng=0, zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("RdYlBu", goal_3_score)(goal_3_score), label = mytext)
