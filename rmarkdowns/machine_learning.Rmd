...
gg_miss_var(sdr_data_normalized_scores_no_na_countries, show_pct = TRUE) +
  theme(axis.text.y = element_text(size = 8)) +
  geom_hline(yintercept = 20, color = "yellow", linetype = "dashed")
```
...
ggplot(importance_df_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  ...
  theme_classic() +
  labs(title = "Most Important Variables Predicting Maternal Mortality",
       subtitle = "Top 10",
       y = "SDG Indicator",
       x = "Feature Importance (% Increase in Mean Squared Error)")
```
...
- Remember, this is a general trend for all countries in the dataset
...
```
```{r}
ph <- pdp::partial(rf_literacy, pred.var = c("normalized_score_sdg3_uhc", "normalized_score_sdg3_neonat"))
plotPartial(ph)
```
