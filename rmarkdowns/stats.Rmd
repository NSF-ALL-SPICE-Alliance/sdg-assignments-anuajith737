...
```
```{r}
ggplot(sdr_data, aes(x = goal_7_score, y = goal_1_score)) + geom_point() + theme_minimal()
```
...
sdr_scores <- sdr_data %>%
  select(
    goal_1_score, goal_2_score, goal_3_score, goal_4_score, goal_5_score, goal_6_score, goal_7_score, goal_8_score,
    goal_9_score, goal_10_score, goal_11_score, goal_12_score, goal_13_score, goal_14_score,
...
```{r}
ggcorrplot(cor, method = "square", type = "lower", lab = TRUE, lab_size = 2) +
  theme(axis.title.y = element_text(size = 8),
        axis.text.x = element_text(size = 8))
```
```{r}
ggcorrplot(cor, method = "circle", type = "lower", lab = TRUE, lab_size = 2) +
  theme(axis.title.y = element_text(size = 8),
        axis.text.x = element_text(size = 8)) +
  labs(y = NULL)  # Removes the 'Var2' label on the y-axis
...
ggplot(sdr_data, aes(x = goal_3_score, 
  ...
  stat_cor() +
  theme_minimal() 
goals_correlation_plot <- ggplot(sdr_data, aes(x = goal_3_score, 
                     y = goal_6_score, label = country)) +
  geom_point() +
  geom_smooth() +
  stat_cor() +
